$(window).load(function() {
    $.ajax({
        ...
        success: function(datos)
        {
            ...
            $.each(objeto, function(key, item)
            {
                ...
                texto += "<tr class=" + objeto[key].id + "><td>" + objeto[key].nombre + "</td><td>" + objeto[key].region + "</td>";
                texto += "<td>" +
    ...
    $('#btnNuevoAceptar').click(function()
    {
        ...
        $.ajax({
            ...
            data:
                    {
                        funcion: "nueva_ubicacion",
                        nombre: $('#Nueva_ubicacion_nombre').val(),
                        region: $('#nueva_region').val()
                    },
            success: function(datos)
            {
                //alert(datos);
    ...
    $.ajax({
        ...
        success: function(datos) {
            ...
            $('#nueva_region').html(texto);
            $('#mNueva_region').html(texto);
            //$('#mestados').html(texto);
    ...
    $('#btnEditarAceptar').click(function()
    {
        ...
        $.ajax({
            ...
            data: {
                funcion: "updt_ubicacion",
                id: $('#mid').val(),
                nombre: $('#mNueva_region_nombre').val(),
                region: $('#mNueva_region').val()
            },
            success: function(datos) {
                //alert(datos);
...
{
    $region = $('.' + id).find('td:eq(1)').text();

    $('#mid').val(id);
    $('#mNueva_region_nombre').val($('.' + id).find('td:eq(0)').text());
    $('#mNueva_region').val($('.' + id).find('td:eq(1)').text());
    $("#mNueva_region option:contains(" + $region + ")").prop('selected', true);
    $('#dlgModificar').modal('show');
}

function dlgEliminar(id)
{
    var r = confirm("¿Está seguro que desea eliminar la ubicacion " + $('.' + id).find('td:eq(0)').text());
    if (r == true) {
        ...
        $.ajax({
            ...
            data:
                    {
                        funcion: "get_dependencia_ubicacion",
                        id: id
                    },
            success: function(datos) {
                //alert(datos);
                ...
                {
                    alert('No es posible eliminar la ubicación pues se encuentra asignada a un subproyecto.');
                }
            }
        });
    }
