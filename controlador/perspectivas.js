...
$(window).load(function(){
	$.ajax({
		type: "POST",
		url: "modelo/perspectivas.php",
		data: {
			funcion: "Listar_perspectivas"	
		},
		success: function(datos){
			...
			$.each( objeto , function(key, item){
				...
				texto += "<tr class="+objeto[key].id+"><td>"+objeto[key].descripcion+"</td><td>"+objeto[key].nombre_estados+"</td>";
				texto += "<td>"+						
	...
	$.ajax({
        ...
        success: function(datos) {
            ...
            $('#Nueva_perspectiva_estado').html(texto);
            $('#mNueva_perspectiva_estado').html(texto);
	...
	$('#btnNuevoAceptar').click(function()
	{
		...
		{*/
		if($('#Nueva_perspectiva_nombre').val().length >0)
		{	
			$.ajax({
				type: "POST",
				url: "modelo/perspectivas.php",
				data: 
				{
					funcion: "nueva_perspectiva",
					nombre: $('#Nueva_perspectiva_nombre').val(),
					estado: $('#Nueva_perspectiva_estado').val()
				},
				success: function(datos)
				{
					//alert(datos);
	...
	{
		if($('#mNueva_perspectiva_nombre').val().length >0 )
		{
			$('.mensajes').css('display','none');
				$.ajax(
				{
					...
					data: 
					{
						funcion: "updt_perspectiva",
						id: $('#mid').val(),
						nombre: $('#mNueva_perspectiva_nombre').val(),
						estado: $('#mNueva_perspectiva_estado').val()
					},
				success: function(datos)
				{
					//alert(datos);
	...
	function dlgModificar(id)
{
	...
	$('#mid').val(id);
	$('#mNueva_perspectiva_nombre').val( $('.'+id).find('td:eq(0)').text() );
	$("#mNueva_perspectiva_estado option:contains(" + $estado + ")").prop('selected', true);
