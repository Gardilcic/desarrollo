...
$(window).load(function() {
    ...
    $.ajax({
        type: "POST",
        url: "modelo/carga_itemizado_pmo.php",
        data: {
        ...
        success: function(datos) {
            ...
            $.each(objeto, function(key, item) {
                ...
                texto += "<tr class=" + objeto[key].id + " ><td>" +
                        ...
                        objeto[key].estado_nombre + "</td><td>" +
                        objeto[key].nombre + "</td>";
    ...
    /*$.ajax({
     ...
     $('#nom_version').html(texto);
     }
     });*/

    $('#xls_empresa').change(function() {
        $.ajax({
            type: "POST",
            url: "modelo/carga_itemizado_pmo.php",
            data: {
                funcion: "listar_versiones",
                empresa: $('#xls_empresa').val()
            },
            success: function(datos)
            {
                var objeto = eval(datos);
    ...
    $('#empresas').change(function() {
        $.ajax({
            ...
            url: "modelo/carga_itemizado_pmo.php",
            data: {
                funcion: "listar_versiones_nuevo",
    ...
    $('#nversion').change(function() {
        $.ajax({
            type: "POST",
            url: "modelo/carga_itemizado_pmo.php",
            data: {
                funcion: "ListarItemizadoPMO_nuevo",
                empresa: $('#empresas').val(),
                version: $('#nversion').val()
            },
            success: function(datos)
            {
                //alert(datos);
                var objeto = eval(datos);
                var texto = "<option value='0'>Nuevo Padre</option>";

                $.each(objeto, function(key, item) {
                    texto += "<option value=" + objeto[key].id + ">" + objeto[key].codigo + " " + objeto[key].descripcion + "</option>";
                });
    ...
    $('#padre').change(function() {
        ...
        $.ajax({
            ...
            data: {
                ...
                id_version: $('#nversion option:selected').val()
            },
            success: function(datos) {
                //alert(datos);
                var objeto = eval(datos);
    ...
    $('#btnNuevoAceptar').click(function() {
        ...
        if ($('#ndescripcion').val().length > 0 && $('#nfactor').val() > 0 && $('#nunidades').val() > 0 &&
                $('#nestado').val() > 0 && $('#codigo').val() > 0) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    factor: $('#nfactor').val(),
                    idpadre: $('#padre').val(),
                    idunidad: $('#nunidades').val(),
                    idestado: $('#nestado').val(),
        ...
        if ($('#mdescripcion').val().length > 0 && $('#mfactor').val().length > 0 &&
            ...
            $.ajax({
                ...
                success: function(datos) {
                    ...
                    if (datos > 0)
                    {
                        alert(decodeURIComponent('Acci�n realizada. Gracias'));
                        location.reload();

                    }
                    else
                    {
                        alert(decodeURIComponent(('Por alguna razón su solicitud no se pudo completar.')));
                    }
                }
            });
        }
    ...
    $('#btnEliminarAceptar').click(function() {
        ...
        $.ajax({
            ...
            url: "modelo/carga_itemizado_pmo.php",
            data: {
                funcion: "Eliminar_itm",
                itemizado: $('#mIdRegistro').val()
            },
            success: function(datos) {
                if (datos == 0)
                {
                    alert(decodeURIComponent('Acci�n realizada.'));
                    $('#dlgEliminar').modal('hide');
                    location.reload();
                }
                else
                {
                    alert(decodeURIComponent('Lamentablemente no ha sido posible realizar la operaci�n por existir relaciones con este itemizado.'));
                }
            }
        });
    });
