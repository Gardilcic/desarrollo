...
{
	var r=confirm("Está seguro que desea eliminar la unidad " + $('.'+id).find('td:eq(0)').text());
	if(r==true)
	{
		...
		$.ajax(
		{
			...
			success: function(datos)
			{
				...
				{
					alert('No es posible eliminar la unidad de registro pues se encuentra asignada.');
				}
			}
		});
	}
