$(window).load(function() {
            ...
            $.ajax(
                    {
                        ...
                        success: function(datos)
                        {
                            ...
                            {
                                alert('Por alguna raz�n su solicitud no pudo realizarse. Por favor revise los datos enviados.');
                            }
                        }
                    });
        }
    ...
    $('#btnAsociarAceptar').click(function() {
        ...
        $.ajax({
            ...
            success: function(datos) {
                ...
                {
                    alert('No ha sido posible asignarle este perfil. Probablemente tenga un perfil asignado a su usuario.');
                }
            }
        });
    });
    ...
    $('#btnAsignarSubProyectoAceptar').click(function() {
        $('.mensajes').css('display', 'none');
        $.ajax({
            type: "POST",
            url: "modelo/usuarios.php",
            async: false,
            data: {
                funcion: "BorrarUsuarioSubproyecto",
                idUsuario: $('#ausuarios').val()
            },
            success: function(datos) {
                //alert(datos);
        ...
        $.each($('#subproyectos option.sombreado'), function() {
            $.ajax({
                type: "POST",
                url: "modelo/usuarios.php",
                data: {
                    funcion: "AsociarUsuarioSubproyecto",
                    idUsuario: $('#susuarios').val(),
                    idSubproyecto: $(this).attr('value')
                },
                success: function(datos) {
                    //alert(datos);
                    if (datos == 0) {
...
{
    var r = confirm("Está seguro que desea eliminar el perfil " + $('.' + id).find('td:eq(0)').text());
    if (r == true)
    {
        //alert('ID:'+id);
        $.ajax(
