...
$(window).load(function(){
	$.ajax({
		...
		data: {
			funcion: "Listar_subclasificacion"	
		},
		success: function(datos){
			...
			$.each( objeto , function(key, item){
				...
				texto += "<tr class="+objeto[key].id+"><td>"+objeto[key].nombre+"</td><td>"+objeto[key].nombre_clasificacion+"</td><td>"+objeto[key].nombre_estados+"</td>";
				texto += "<td>"+						
	...
	$.ajax({
        ...
        success: function(datos) {
            ...
            $('#Nueva_subclasificacion_estado').html(texto);
            $('#mNueva_subclasificacion_estado').html(texto);
	...
	$('#btnNuevoAceptar').click(function()
	{
		...
		{*/
			$.ajax({
				...
				data: 
				{
					funcion: "nueva_subclasificacion",
					nombre: $('#Nueva_subclasificacion_nombre').val(),
					clasificacion: $('#Nueva_subclasificacion_clasificacion').val(),
					estado: $('#Nueva_subclasificacion_estado').val()
				},
				success: function(datos)
				{
					//alert(datos);
	...
	{
		if($('#mNueva_subclasificacion_nombre').val().length >0)
		{
			$('.mensajes').css('display','none');
				$.ajax(
				{
					...
					data: 
					{
						funcion: "updt_subclasificacion",
						id: $('#mid').val(),
						nombre: $('#mNueva_subclasificacion_nombre').val(),
						clasificacion: $('#mNueva_subclasificacion_clasificacion').val(),
						estado: $('#mNueva_subclasificacion_estado').val()
					},
				success: function(datos)
				{
	...
	$('#btnEliminarAceptar').click(function(){
		$('.mensajes').css('display','none');
			...
			$.ajax({
				...
				url: "modelo/subclasificacion.php",
				data: {
					funcion: "eliminar_subclasificacion",
					//id: $('#mid').val()
					id: $('#IdRegistro').val()
				},
				success: function(datos){
	...
	function dlgModificar(id)
{
	...
	$('#mid').val(id);
	$('#mNueva_subclasificacion_nombre').val( $('.'+id).find('td:eq(0)').text() );
	$("#mNueva_subclasificacion_clasificacion option:contains(" + $clasificacion + ")").prop('selected', true);
	$("#mNueva_subclasificacion_estado option:contains(" + $estado + ")").prop('selected', true);
