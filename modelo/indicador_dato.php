...
if ($_POST['funcion'] == "Listar_indicadores") {
    ...
    $var1 = "select id, descripcion, detalle_nivel, tolerancia1, tolerancia2, tipo,  nombre, abreviacion, nombre_estados, descripcion_nivel, id_Nivel from vista_indicador_dato order by descripcion asc";
...
if ($_POST['funcion'] == "nuevo_indicador") {
    ...
    $cns.="," . $nivel . "," . $tole1 . "," . $tole2 . "," . $_POST['tipo'] . "," . $_POST['estado'] . ")";
    $cns.="";
    //echo $cns;
    $a = get_datos($cns);
...
if ($_POST['funcion'] == "GrabarFormula") {
    //echo "ListarUsuarios";
    
    $var1 = "DELETE FROM [dbo].[formulas] WHERE id_indicador = $_POST[indicador];";    
    $a = get_datos_sigp($var1);
    $var1 = "INSERT INTO [dbo].[formulas] VALUES ($_POST[indicador],'$_POST[formula]','$_POST[formula_html]');";    
    $a = get_datos_sigp($var1);
    
    $var1 = "UPDATE [dbo].[indicador_dato] SET formula = '$_POST[formula]' WHERE id = $_POST[indicador]";
    $a = get_datos_sigp($var1);    
    $var1 = "SELECT @@ROWCOUNT as respuesta";    
    $a = get_datos_sigp($var1);
...
if ($_POST['funcion'] == "ObtenerFormula") {
    
    $var1 = "SELECT id_indicador, formula, formula_html FROM [dbo].[formulas] WHERE id_indicador = $_POST[indicador]";    
    $a = get_datos_sigp($var1);
