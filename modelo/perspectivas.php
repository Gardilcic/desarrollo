...
if($_POST['funcion']=="Listar_perspectivas"){
		
	$var1 = "select id, descripcion, nombre_estados from vista_perspectivas order by descripcion asc" ;	
...
if($_POST['funcion']=="nueva_perspectiva"){
		//echo "ListarUsuarios";
		
		$cns = "insert into perspectiva (descripcion, id_estados) values ('".decoder($_POST['nombre'])."',".$_POST['estado'].")";
		//echo $cns;
		$a = get_datos($cns);
...
if($_POST['funcion']=="updt_perspectiva")
{
	$cns = "update perspectiva set descripcion='".decoder($_POST['nombre'])."', id_estados=".$_POST['estado']." where id=".$_POST['id'];
	//echo $cns;
	$a = get_datos($cns);
}
...
if($_POST['funcion']=="eliminar_perspectiva")
{
	$respuesta = 0;
	$error = 5;
	$cns0="delete from perspectiva where id =".$_POST['id'];
	//echo $cns0;
	/*if(valida_dependencias($_POST['id']) == 0)
	{*/
		$a=get_datos($cns0);
		$cns="select count(id) as cant from perspectiva where id =".$_POST['id'];
		$a=get_datos($cns);
		while(odbc_fetch_row($a))
		{
