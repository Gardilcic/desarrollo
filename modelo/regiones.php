<?php
	...
	{
		$cns = "insert into regiones (nombre, simbolo, id_pais) values('".decoder($_POST['nombre'])."','".decoder($_POST['abb'])."',".$_POST['pais'].")";
	...
	if($_POST['funcion']=="GrabarNuevoUsuario"){
		...
		$cns = "INSERT INTO sigp_usuarios.dbo.usuarios (nombre,apellidos,rut,clave,id_estados,id_perfil) VALUES ".
           		"('".decoder($_POST['nombre'])."','".decoder($_POST['apellidos'])."','".$_POST['rut']."','".decoder($_POST['clave'])."','".$_POST['idEstado']."','".$_POST['idPerfil']."')";
...
{
	$respuesta = 0;
	$cns0 = "select count(id) as cant from ubicaciones where region=".$_POST['region'];
...
if($_POST['funcion']=="borra_region")
{
	$respuesta = 0;
	
	$cns0 = "delete from regiones where id=".$_POST['region'];
	
	$a = get_datos($cns0);
	
	$cns0 = "select COUNT(id) as cant from regiones where id=".$_POST['region'];
	$a = get_datos($cns0);
