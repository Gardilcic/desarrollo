...
if($_POST['funcion']=="Listar_subclasificacion"){
		
	$var1 = "select id, nombre, nombre_clasificacion, nombre_estados from vista_subclasificacion order by nombre asc" ;	
...
if($_POST['funcion']=="nueva_subclasificacion"){
		//echo "ListarUsuarios";
		
		$cns = "insert into subclasificacion (nombre, id_clasificacion, id_estados) values ('".decoder($_POST['nombre'])."',".$_POST['clasificacion'].",".$_POST['estado'].")";
		//echo $cns;
		$a = get_datos($cns);
...
if($_POST['funcion']=="updt_subclasificacion")
{
	$cns = "update subclasificacion set nombre='".decoder($_POST['nombre'])."', id_clasificacion=".$_POST['clasificacion'].", id_estados=".$_POST['estado']." where id=".$_POST['id'];
	//echo $cns;
	$a = get_datos($cns);
}
...
if($_POST['funcion']=="eliminar_subclasificacion")
{
	$respuesta = 0;
	$error = 5;
	$cns0="delete from subclasificacion where id =".$_POST['id'];
	//echo $cns0;
	if(valida_dependencias($_POST['id']) == 0)
	{
		$a=get_datos($cns0);
		$cns="select count(id) as cant from subclasificacion where id =".$_POST['id'];
		$a=get_datos($cns);
		while(odbc_fetch_row($a))
		{
